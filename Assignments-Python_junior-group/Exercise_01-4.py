...
x = y = 0
h = l = 50
for i in range(10):  # создаем горизонтальный ряд квадратов
    for ii in range(10):  # создаем вертикальный ряд квадратов
        pygame.draw.rect(screen, (0, 0, 0), (x, y, h, l), 1)
        text = f.render(str(n), True, (180, 0, 0))
        screen.blit(text, (x+5, y+7))
        n += 1
        y += 50 # сместим кординату y для следующего квадрата столбца
    x += 50  # сместим кординату x для следующего горизонтального ряда
    y = 0  # в новый вертикальный ряд квадратов начнем с координаты y = 0
